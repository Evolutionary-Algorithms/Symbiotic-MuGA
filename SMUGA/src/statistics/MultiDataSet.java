...
//                data[i] = (XYSeries) ((Solver) ga.get(i)).getStats().getDataSet(index).clone();
    ...
    public void saveImage(String path) {
        try {
            ...
            while (trysToSave-- > 0) {
                try {
                    saveChart(path + "/original/", fileName, saveChart, panel.getWidth(),  panel.getHeight());
                    saveChart(path + "/Square/", fileName, saveChart, imgSize,  imgSize);
                    saveChart(path + "/low/", fileName, saveChart,300,  300);
                    saveChart(path + "/lowV/", fileName, saveChart,300,  500);
                    saveChart(path + "/lowH/", fileName, saveChart,500,  300);
                    saveChart(path + "/normal/", fileName, saveChart,500,  500);
                    saveChart(path + "/normalH/", fileName, saveChart,1000,  500);
                    saveChart(path + "/normalV/", fileName, saveChart,500,1000);
                    saveChart(path + "/High/", fileName, saveChart,1000,1000);
                    saveChart(path + "/HighH/", fileName, saveChart,2000,1000);
                    saveChart(path + "/HighV/", fileName, saveChart,1000,2000);
                    //sucess
                    return;
    ...
    private void saveChart(String path, String fileName, JFreeChart saveChart, int width, int height) throws IOException {
            //make path
            File folder = new File(path);
            folder.mkdirs();
            File image = new File(folder, fileName + ".png");
            ChartUtilities.saveChartAsPNG(image, saveChart, width, height);
    }
