...
//        Collections.sort(sort);
     ...
     public Iterable<Individual> getRandomIterable() {
        ArrayList<Individual> rndList = new ArrayList<Individual>(size());
        Iterator<Individual> iter = getIterator();
        while (iter.hasNext()) {
            rndList.add(iter.next());
        }
    ...
    public void reScaleCopys(double factor) {
        ...
        int max = 1 + (int) (getNumGenotypes() * 0.2);
        while (elems.hasNext()) {
            //element of population
            Individual elem = elems.next();
