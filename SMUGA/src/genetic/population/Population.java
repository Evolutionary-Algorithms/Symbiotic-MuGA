...
public abstract class Population implements Serializable {
    ...
    public abstract Iterable<Individual> getRandomIterable();
    ...
    /**
     * return an sorted array of individuals
     * @return 
     */
    public ArrayList<Individual> getSortedArray(){
        ArrayList<Individual> list = new ArrayList<>();
        Iterable<Individual> it = getSortedIterable();
        for(Individual i: it)
            list.add(i);
        return list;
    }
    ...
    public void createPopulation(int sizePop, Individual template) {
        clear();
        int trys = sizePop *10;
        while (trys >0 && size() < sizePop) {
            addRandom(template);
            trys--;
        }        
        cleanCopies();
    ...
    public void cleanCopies(){
        Iterator<Individual> it = getIterator();
        while(it.hasNext())
            it.next().setNumCopys(1);
    }
    public void setFitness(double value){
        Iterator<Individual> it = getIterator();
        while(it.hasNext())
            it.next().setFitness(value);
    }
    ...
    public void createRandomPopulation(int sizePop, Individual template) {
        ...
        } else if (template instanceof PRM_Individual || template instanceof Parasite) {
            generator = null;
        } else {
