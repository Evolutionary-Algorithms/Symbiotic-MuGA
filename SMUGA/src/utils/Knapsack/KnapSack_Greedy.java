...
public class KnapSack_Greedy {
    ...
    public static double MAX_PROBABILITY = 0.9;
    public static double MIN_PROBABILITY = 0.1;
...
//                System.out.println(i + "  " + prob);
    ...
    public static boolean[] greedyFillSigmoid(double[] profit, double[] weight, double sackSize) {
        ...
        double prob;
        double beta =  weight.length;//Math.sqrt(weight.length);
        for (int i = 0; i < bits.length; i++) {
            //sigmoid Function
            prob = 1.0 / (1 + Math.exp(-beta * (sackSize/totalSize )));
