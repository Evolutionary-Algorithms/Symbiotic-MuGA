...
//        // set the background color for the chart...
    ...
    public static void saveChart(double[][] values, String[] algorithms, String statistic, String path, int size) {
        try {
            ...
            File image = new File(dir, fileName + ".png");
            ChartUtilities.saveChartAsPNG(image, chart, size, size);
            dir = new File(path + "boxPlot/"+size+"/vertical/");
            dir.mkdirs();
            image = new File(dir,  fileName + ".png");
            ChartUtilities.saveChartAsPNG(image, chart, size, size*2);
            dir = new File(path + "boxPlot/"+size+"/horizontal/");
            dir.mkdirs();
            image = new File(dir, fileName + ".png");
            ChartUtilities.saveChartAsPNG(image, chart, size*2, size);
