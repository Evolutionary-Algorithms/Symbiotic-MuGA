 ...
 * @author arm
 */
public class M_FlipBitWave extends Mutation {
    //lenght of the wave for probability

    static double DEFAULT_EXP = 3;
    public static double EXPONENT = DEFAULT_EXP;
    //lenght of the wave for probability
    static double DEFAULT_WAVE = 2;
    static double WAVE = DEFAULT_WAVE;

    public static double waveFunctionSin(Individual ind) {
        return waveFunctionSin(ind.getNumCopies(), WAVE);
    }

    public static double waveFunctionSin(int copies) {
        return waveFunctionSin(copies, WAVE);
    }

    private static double waveFunctionSin(int copies, double exp) {
        double val = (-Math.sin(Math.PI / 2 + (copies - 1) / WAVE) + 1) / 2.0;
    ...
    public void doMutation(Individual ind) {
        ...
        double prob = probability == 0 ? 1.0 / (double) bits.getNumberOfBits() : probability;
        prob += waveFunctionSin(ind.getNumCopies(), EXPONENT);
    ...
    public Mutation getClone() {
        M_FlipBitWave mut = (M_FlipBitWave) super.getClone();
        mut.WAVE = this.WAVE;
