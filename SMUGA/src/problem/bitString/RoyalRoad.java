...
//            addGene(new GeneBinary(SIZE_OF_BLOCK));
    ...
    public String toStringPhenotype() {
        ...
        for (int i = 0; i < getNumGenes(); i++) {
            txt.append("[" + getGeneValue(bits, i) + "(" + getGeneBits(bits, i) + ")]");
        }
        return txt.toString();
    ...
    protected BitField getGeneBits(BitField bits, int gene) {
        return bits.getSubBitField(gene * SIZE_OF_BLOCK, (gene + 1) * SIZE_OF_BLOCK -1);
    }

    public double getGeneValue(BitField bits, int index) {
        BitField gene = getGeneBits(bits, index);
        return gene.getNumberOfOnes() == SIZE_OF_BLOCK ? SIZE_OF_BLOCK : 0;
    }
...
//    }
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 100; i++) {
            RoyalRoad b = new RoyalRoad();
            b.evaluate();
            System.out.println(b.toStringGenotype());
            System.out.println(b.toStringPhenotype());
