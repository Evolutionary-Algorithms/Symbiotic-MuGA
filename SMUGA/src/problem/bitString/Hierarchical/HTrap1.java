...
public class HTrap1 extends Individual {
    ...
    // Level = 7 = 2187 bits
    // Level = 8 = 6561 bits
   public static int []size = {9,37,81,243,729,2187,6561};
    ...
    public boolean isBest() {
        return getStringBits().getNumberOfOnes() == SIZE;
    }
...
//        System.out.println(level + "\t BITS = " + bits);
        ...
        for (int i = 0; i < bits.length(); i += 3) {
            ...
            if (bits.charAt(i) == '0' && bits.charAt(i + 1) == '0' && bits.charAt(i + 2) == '0') {
                newLevel.append("0");
                sum += trap3(0.9, 1.0, bits.charAt(0), bits.charAt(1), bits.charAt(2));
            } //true
    ...
    public String toStringPhenotype() {
        ...
        txt.append(" = "); 
        txt.append( HTrap3(getStringBits().toString()));
